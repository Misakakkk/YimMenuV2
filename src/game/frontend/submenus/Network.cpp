...
namespace YimMenu::Submenus
{
	Network::Network() :
	    ...
	    Submenu::Submenu("网络选项", ICON_FA_ROUTE)
	{
		...
		joinSession->AddItem(std::make_shared<ListCommandItem>("joinsessiontype"_J, "战局类型"));
		...
		joinGroup->AddItem(std::make_shared<ImGuiItem>([] {
			...
			ImGui::InputText("昵称", name_buf, sizeof(name_buf));
			ImGui::SameLine();
		...
		bountyGroup->AddItem(std::make_shared<IntCommandItem>("bountyamount"_J, "金钱"));
		...
		tpToProperty->AddItem(std::make_shared<CommandItem>("sendtopropertyall"_J, "全部发送到房产"));
		...
		tpToInterior->AddItem(std::make_shared<CommandItem>("sendtointeriorall"_J, "全部发送到室内"));
		teleportGroup->AddItem(tpToProperty);
		teleportGroup->AddItem(tpToInterior);
		teleportGroup->AddItem(std::make_shared<CommandItem>("bringall"_J, "全部带来"));
		...
		toxicGroup->AddItem(std::make_shared<CommandItem>("explodeall"_J, "所有人爆炸"));
		toxicGroup->AddItem(std::make_shared<CommandItem>("ceokickall"_J, "踢出所有CEO"));
		toxicGroup->AddItem(std::make_shared<CommandItem>("hkickall"_J, "所有玩家房主踢出"));
		...
		auto spoofing = std::make_shared<Category>("服务");
